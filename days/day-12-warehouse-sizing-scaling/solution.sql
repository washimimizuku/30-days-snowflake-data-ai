/*
Day 12: Warehouse Sizing & Scaling - Solution
...
-- BI warehouse
CREATE WAREHOUSE IF NOT EXISTS bi_wh WITH
...
-- Multi-cluster with Standard policy
CREATE WAREHOUSE IF NOT EXISTS multi_standard_wh WITH
  WAREHOUSE_SIZE = 'MEDIUM'
  MIN_CLUSTER_COUNT = 1
  MAX_CLUSTER_COUNT = 3
...
-- Multi-cluster with Economy policy
CREATE WAREHOUSE IF NOT EXISTS multi_economy_wh WITH
  WAREHOUSE_SIZE = 'MEDIUM'
  MIN_CLUSTER_COUNT = 1
  MAX_CLUSTER_COUNT = 3
...
-- View multi-cluster configuration
...
SELECT 
  ...
  w.max_cluster_count,
  w.auto_suspend,
  COALESCE(m.total_credits_7d, 0) as total_credits_7d,
  COALESCE(m.estimated_cost_7d_usd, 0) as estimated_cost_7d_usd,
  COALESCE(q.query_count_7d, 0) as query_count_7d,
  COALESCE(q.avg_execution_seconds, 0) as avg_execution_seconds
FROM (
...
CREATE RESOURCE MONITOR IF NOT EXISTS dev_monthly_limit WITH
  CREDIT_QUOTA = 100
  FREQUENCY = MONTHLY
...
-- Undersized warehouses (queuing issues)
SELECT 
  warehouse_name,
...
SELECT 
  ...
  queued_queries,
  CASE 
    WHEN queued_queries > 100 THEN 'Consider scaling up or out'
    WHEN query_count < 50 AND total_credits > 10 THEN 'Consider downsizing'
    WHEN total_credits / NULLIF(query_count, 0) > 1 THEN 'Review efficiency'
...
FROM (
  SELECT 
    ...
    SUM(CASE WHEN qh.queued_overload_time > 0 THEN 1 ELSE 0 END) as queued_queries
  FROM SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY wm
  JOIN SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSES w ON wm.warehouse_name = w.name
...
-- ============================================================================
-- Bonus: Cost Optimization Analysis
-- ============================================================================

-- Potential savings from auto-suspend optimization
SELECT 
  wm.warehouse_name,
  w.auto_suspend,
  SUM(wm.credits_used) as total_credits,
  CASE 
    WHEN w.auto_suspend > 60 THEN 
      ROUND(SUM(wm.credits_used) * 0.1, 2)
    ELSE 0
  END as potential_savings_credits,
  CASE 
    WHEN w.auto_suspend > 60 THEN 
      ROUND(SUM(wm.credits_used) * 0.1 * 3, 2)
    ELSE 0
...
-- Warehouse consolidation opportunities
SELECT 
  warehouse_name,
  COUNT(DISTINCT DATE(start_time)) as active_days,
  SUM(credits_used) as total_credits,
  COUNT(*) as query_count,
  CASE 
    WHEN active_days < 10 AND query_count < 100 THEN 'Consider consolidation'
    ELSE 'Keep separate'
  END as recommendation
FROM SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY
WHERE start_time >= DATEADD(day, -30, CURRENT_TIMESTAMP())
