/*
Day 4: Streams + Tasks Integration - Solution
...
-- Create a task that processes the stream
...
-- Should show: order 1 deleted, order 3 updated, order 5 inserted
...
-- Create stream on customers
CREATE STREAM customers_stream ON TABLE customers;

-- Create task for SCD Type 2 processing
...
-- Update a customer to trigger SCD
UPDATE customers 
...
-- Should show 2 records: old (gold, is_current=FALSE) and new (platinum, is_current=TRUE)
...
-- Create root task - Extract to staging
...
-- Create child task - Transform and load to final
...
-- Create child task - Create summary
...
INSERT INTO orders (order_id, customer_id, product_id, quantity, amount, order_status, order_date) VALUES
  (8, 108, 1008, -1, 99.99, 'pending', '2025-12-05'),  -- Invalid quantity
  (9, 109, 1009, 1, -50.00, 'pending', '2025-12-05');  -- Invalid amount

ALTER TASK process_with_errors RESUME;
EXECUTE TASK process_with_errors;

-- Check DLQ
SELECT * FROM dlq_orders;

ALTER TASK process_with_errors SUSPEND;
...
-- Create monitoring view for stream status
...
-- Query stream monitoring
SELECT * FROM stream_monitoring;

-- Create view for task execution summary
...
-- Query task execution summary
SELECT * FROM task_execution_summary LIMIT 10;

-- Create pipeline health check
SELECT 
...
-- ============================================================================
-- Bonus: Production-Ready Pipeline
-- ============================================================================

CREATE OR REPLACE STREAM production_stream ON TABLE orders;
...
AS
BEGIN
  -- Merge with full error handling
  MERGE INTO orders_replica AS target
  USING (
    SELECT * FROM production_stream
    WHERE amount > 0 AND quantity > 0  -- Validation
  ) AS source
  ON target.order_id = source.order_id
  ...
  WHEN NOT MATCHED AND source.METADATA$ACTION = 'INSERT' THEN
    INSERT VALUES (source.order_id, source.customer_id, source.product_id, 
                   source.quantity, source.amount, source.order_status, 
                   source.order_date, source.last_updated);
  
  -- Log invalid records
  INSERT INTO dlq_orders (original_record, error_reason)
  SELECT OBJECT_CONSTRUCT(*), 'Validation failed'
...
EXCEPTION
  WHEN OTHER THEN
    INSERT INTO cdc_error_log (task_name, error_message)
    VALUES ('production_cdc_pipeline', SQLERRM);
END;
...
ALTER TASK summarize_orders SUSPEND;
ALTER TASK process_with_errors SUSPEND;
ALTER TASK production_cdc_pipeline SUSPEND;
...
-- Drop views
DROP VIEW IF EXISTS stream_monitoring;
DROP VIEW IF EXISTS task_execution_summary;
