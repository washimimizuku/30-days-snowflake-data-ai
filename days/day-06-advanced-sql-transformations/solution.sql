/*
Day 6: Advanced SQL Transformations - Solution
...
-- Assign customers to quartiles based on spending
SELECT 
  customer_id,
  SUM(amount) as total_spent,
...
-- Get top 2 products per category by sales
SELECT 
  category,
  product_id,
...
-- Deduplicate - keep most recent sale per customer
SELECT 
  customer_id,
  sale_id,
  sale_date,
...
-- Get top 3 sales for each customer
SELECT 
  c.customer_id,
  c.customer_name,
  s.sale_id,
...
-- Calculate customer statistics with LATERAL
SELECT 
  c.customer_id,
  c.customer_name,
...
-- Generate date series for December 2025
SELECT 
...
-- Complete customer analytics query combining multiple techniques
SELECT 
  c.customer_id,
  c.customer_name,
  c.customer_tier,
...
-- Example: Pivot data (category sales by customer tier)
SELECT *
FROM (
  SELECT 
    c.customer_tier,
...
-- Example: Gap detection (find missing dates)
WITH date_series AS (
  SELECT 
    DATEADD(day, SEQ4(), '2025-12-01'::DATE) as date
...
-- Example: Cumulative distinct count
SELECT 
  sale_date,
  customer_id,
...
SELECT 
  customer_id,
  SUM(amount) as total_spent,
  PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY SUM(amount)) OVER () as median_spent,
  PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY SUM(amount)) OVER () as p75_spent,
  PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY SUM(amount)) OVER () as p90_spent
FROM sales
GROUP BY customer_id
ORDER BY total_spent DESC;
